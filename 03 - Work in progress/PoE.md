...
Power over Ethernet allows a power source to be sent over ethernet concurrent with data. It has two methods of sending power. Phantom Powering is the term used when power, as well as data pulses can be sent over the same copper cable. There are two modes, 
